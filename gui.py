...
# icon= tkinter.PhotoImage(file="D:\Family\group.jpg")
# label = tkinter.Label(root, image = icon)
# label.pack()
headline=tkinter.Label(root,text="Login Credentials",bg="green",fg="black",highlightbackground="grey",highlightthickness=4,highlightcolor="black").grid(row=0,column=1)

lbl = tkinter.Label(root, text="Enter something:")
lbl.grid(column=0, row=2)

# Add an entry box for user input
txt = tkinter.Entry(root, width=20)
...
def clicked():
    user_input = txt.get()
    if user_input == "":
        lbl.configure(text="Please type something!")
    else:
        lbl.configure(text=f"You wrote: {user_input}")
        txt.destroy()
        btn.destroy()
# Add a button
btn = tkinter.Button(root, text="Submit",fg='green', command=clicked)
...
# tkinter.Button(window,text="Login").grid(column=1)
# window.mainloop()
